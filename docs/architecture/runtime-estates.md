...
- `qaDebugApi.ts` — `window.__KAOS_DEBUG__` mutation API
- `scenarioStateController.ts` — scenario step mutation
- `vfsTortureTest.ts` — VFS stress test
- `ports/kaosUnsafeQaPort.ts` — unsafe QA port interface
...
src/
  ...
  telemetry/                  Observer entry + host interface
    ...
    debug/ports/
      kaosDebugPort.ts        KaosDebugPort interface (canonical — used by QA + tests)
...
kaos.zip      Convenience zip of the above
```
