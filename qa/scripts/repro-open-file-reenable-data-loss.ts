 ...
 *   "I had the note open, turned KAOS off, edited the note, turned KAOS
 ...
 *   4. Disable KAOS on B. Editor stays open.
 *   5. A modifies file via KAOS — REMOTE_FROM_A propagates to server.
...
async function main() {
	...
	const bGot = await b.evalRaw<boolean>(`
		(async () => {
			try { await window.__KAOS_DEBUG__.waitForFile(${JSON.stringify(SCRATCH)}, 60000); return true; } catch { return false; }
		})()
	`).catch(() => false);
	if (!bGot) {
	...
	// --- Phase 3: Disable KAOS on B (editor stays open) ---

	log("Phase 3: Disabling KAOS on B (editor remains open)...");
	...
	// --- Phase 4: A appends REMOTE_FROM_A through KAOS ---

	log("Phase 4a: A inserting REMOTE_FROM_A through KAOS...");
	await a.evalRaw(`
		(async () => {
	...
	// --- Phase 6: Re-enable KAOS on B ---

	log("Phase 6: Re-enabling KAOS on B...");
	...
	`).catch(() => false);
	log(`B KAOS ready and reconciled: ${bReady}`);
	...
	const artifactPath = allFiles.find(p => p !== SCRATCH && (p.includes("KAOS") || p.includes("conflict"))) ?? null;
