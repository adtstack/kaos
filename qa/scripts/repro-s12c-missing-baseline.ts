 ...
 *   - KAOS is disabled on Device B before the file has a stored baselineHash
...
async function waitForIdle(client: ObsidianClient, ms = 15000) {
	await client.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(${ms})`).catch(() => {});
}
...
async function main() {
	...
	// simulating the iPad scenario where the file arrives but KAOS hasn't
	...
	const crdtAdmitted = await b.evalRaw<boolean>(`
		(async () => {
			...
			while (Date.now() < deadline) {
				...
				if (!kaos) return false;
				const text = kaos.vaultSync?.getTextForPath?.(${JSON.stringify(SCRATCH)});
	...
	// -----------------------------------------------------------------------
	// Phase 4: B makes a LOCAL disk edit while KAOS is disabled
	// This is the out-of-band edit (simulates Files app on iPad)
	// -----------------------------------------------------------------------

	log("Phase 4: B writing LOCAL_ON_B directly to disk (KAOS disabled)...");
	await b.evalRaw(`
		(async () => {
			const f = app.vault.getFileByPath(${JSON.stringify(SCRATCH)});
	...
	`).catch(() => []);
	log(`All vault files on B: ${JSON.stringify(allVaultFiles.filter(p => p.includes("QA-scratch") || p.includes("conflict") || p.includes("KAOS")))}`);

	const artifactPath = await b.evalRaw<string | null>(`
		(function() {
			const files = app.vault.getMarkdownFiles();
			const a = files.find(f => f.path.includes("s12c-repro") && (f.path.includes("KAOS") || f.path.includes("conflict")));
			return a?.path ?? null;
		})()
	`).catch(() => null);
	...
	if (!diskWon) {
		...
		console.log("Root cause: KAOS never stored a baselineHash on B for this file");
