 ...
 *   "iOS killed/suspended Obsidian before KAOS had a clean shutdown,
 ...
 *   4.  A writes REMOTE_FROM_A through KAOS (propagates to server).
...
async function main() {
	...
	log("Phase 3a: A writing REMOTE_FROM_A through KAOS...");
	await a.evalRaw(`
		(async () => {
	...
	// KAOS may need to be re-enabled if it was in a disabled state
	const bPluginPresent = await b.evalRaw<boolean>(`!!app.plugins.plugins.kaos`).catch(() => false);
	if (!bPluginPresent) {
		log("B KAOS plugin not present — re-enabling...");
