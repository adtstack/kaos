 ...
 * This is: Obsidian process is killed while KAOS is disabled, user edits disk
 * externally, Obsidian relaunches from cold, KAOS re-enables.
 *
 * Test shape:
 *   1.  A creates baseline file. B syncs it. Both have disk + CRDT + baseline hash.
 *   2.  B: disable KAOS plugin.
 *   3.  B: kill the Obsidian process entirely (not just disable plugin).
 *   4.  While B is dead: A edits file via KAOS (REMOTE_FROM_A propagates to server).
 ...
 *   7.  Wait for B's KAOS to re-enable and finish startup reconciliation.
...
async function main() {
	...
	const bGotFile = await b.evalRaw<boolean>(`
		(async () => {
			// Use KAOS waitForFile which polls for CRDT admission
			try {
				await window.__KAOS_DEBUG__.waitForFile(${JSON.stringify(SCRATCH)}, 60000);
			} catch { return false; }
	...
	await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(10000)`).catch(() => {});
	...
	// --- Phase 2: Disable KAOS on B (before killing) ---

	log("Phase 2: Disabling KAOS on B (plugin only, process still alive)...");
	...
	// Wait for KAOS to initialize on relaunch
	log("Waiting for B KAOS to init after cold relaunch...");
	const bKaosReady = await b.evalRaw<boolean>(`
		(async () => {
	...
	`).catch(() => false);
	log(`B KAOS ready after cold relaunch: ${bKaosReady}`);

	// KAOS may have been disabled before kill — re-enable it
	const bPluginPresent = await b.evalRaw<boolean>(`!!app.plugins.plugins.kaos`).catch(() => false);
	if (!bPluginPresent) {
		log("B KAOS plugin not present after relaunch — re-enabling...");
		await b.evalRaw(`app.plugins.enablePlugin("kaos")`);
		await waitMs(5000);
		const bKaosReady2 = await b.evalRaw<boolean>(`
			(async () => {
				const deadline = Date.now() + 30000;
				while (Date.now() < deadline) {
					const d = window.__KAOS_DEBUG__;
		...
		`).catch(() => false);
		log(`B KAOS ready after re-enable: ${bKaosReady2}`);
	}
	...
	const artifactPath = allMarkdownFiles.find(p => p.includes("KAOS conflict") || p.includes("conflict")) ?? null;
