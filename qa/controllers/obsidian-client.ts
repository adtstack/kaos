...
export class ObsidianClient {
	...
	private async exactVisibleCenter(
		selector: string,
		exactText: string | null = null,
	): Promise<{ x: number; y: number }> {
		return this.evalRaw<{ x: number; y: number }>(`
			(() => {
				const selector = ${JSON.stringify(selector)};
				const exactText = ${JSON.stringify(exactText)};
				const visible = Array.from(document.querySelectorAll(selector)).filter((candidate) =>
					candidate instanceof HTMLElement
					&& candidate.getClientRects().length > 0
					&& (exactText === null || candidate.textContent?.trim() === exactText)
				);
				if (visible.length !== 1) {
					throw new Error(
						"expected exactly one visible match: " + selector + "/" + exactText +
						"; observed=" + visible.length
					);
				}
				const element = visible[0];
				if (!(element instanceof HTMLElement)) throw new Error("visible match is not an element");
				if (element instanceof HTMLButtonElement && element.disabled) {
					throw new Error("visible button is disabled: " + exactText);
				}
				element.scrollIntoView({ block: "center", inline: "nearest" });
				const rect = element.getBoundingClientRect();
				if (rect.width <= 0 || rect.height <= 0) {
					throw new Error("visible match has no clickable bounds");
				}
	...
	private async clickPoint(point: { x: number; y: number }): Promise<void> {
		const cdp = this.requireCdpSession();
		await cdp.send("Input.dispatchMouseEvent", {
			type: "mousePressed",
			x: point.x,
	...
	async clickExactVisibleButton(text: string): Promise<void> {
		await this.clickPoint(await this.exactVisibleCenter("button", text));
	}

	async clickExactVisibleModalButton(text: string): Promise<void> {
		await this.clickPoint(await this.exactVisibleCenter(".modal button:not(:disabled)", text));
	}

	async clickKaosDashboardRibbon(): Promise<void> {
		await this.clickPoint(await this.exactVisibleCenter(".kaos-dashboard-ribbon-icon"));
	}

	async clickHandoffRecoveryRowButton(
		fromPath: string,
		targetPath: string,
		buttonText: string,
	): Promise<void> {
		const point = await this.evalRaw<{ x: number; y: number }>(`
			(() => {
				const pathLabel = ${JSON.stringify(`${fromPath} → ${targetPath}`)};
				const buttonText = ${JSON.stringify(buttonText)};
				const rows = Array.from(
					document.querySelectorAll(".kaos-dashboard-handoff-recovery-row")
				).filter((candidate) =>
					candidate instanceof HTMLElement
					&& candidate.getClientRects().length > 0
					&& candidate.querySelector(".kaos-dashboard-path")?.textContent?.trim() === pathLabel
				);
				if (rows.length !== 1 || !(rows[0] instanceof HTMLElement)) {
					throw new Error("expected one visible Recovery row: " + pathLabel + "; observed=" + rows.length);
				}
				const buttons = Array.from(rows[0].querySelectorAll("button")).filter((candidate) =>
					candidate instanceof HTMLButtonElement
					&& candidate.getClientRects().length > 0
					&& candidate.textContent?.trim() === buttonText
				);
				if (buttons.length !== 1 || !(buttons[0] instanceof HTMLButtonElement)) {
					throw new Error("expected one Recovery row button: " + buttonText + "; observed=" + buttons.length);
				}
				const button = buttons[0];
				if (button.disabled) throw new Error("Recovery row button is disabled: " + buttonText);
	...
	async waitForHandoffRecoveryRow(
		fromPath: string,
		targetPath: string,
		present: boolean,
		timeoutMs = 15_000,
	): Promise<void> {
		const pathLabel = `${fromPath} → ${targetPath}`;
		const startedAt = Date.now();
		let lastCount = -1;
		while (Date.now() - startedAt < timeoutMs) {
			lastCount = await this.evalRaw<number>(`
				(() => Array.from(
					document.querySelectorAll(".kaos-dashboard-handoff-recovery-row")
				).filter((candidate) =>
					candidate instanceof HTMLElement
					&& candidate.getClientRects().length > 0
					&& candidate.querySelector(".kaos-dashboard-path")?.textContent?.trim()
						=== ${JSON.stringify(pathLabel)}
				).length)()
			`);
			if ((present && lastCount === 1) || (!present && lastCount === 0)) return;
			await new Promise((resolveDelay) => setTimeout(resolveDelay, 50));
		}
		throw new Error(
			`Recovery row ${present ? "did not appear" : "did not disappear"}: ${pathLabel}; observed=${lastCount}`,
		);
	}

	async fillVisibleHandoffRecoveryExportPath(path: string): Promise<void> {
		const focused = await this.evalRaw<boolean>(`
			(() => {
				const visible = Array.from(
					document.querySelectorAll(".kaos-handoff-recovery-export-path")
				).filter((candidate) =>
					candidate instanceof HTMLInputElement
					&& candidate.getClientRects().length > 0
				);
				if (visible.length !== 1) {
					throw new Error("expected exactly one visible Recovery export path input; observed=" + visible.length);
				}
		...
		`);
		if (!focused) throw new Error("Recovery export path input did not retain focus");
		await this.requireCdpSession().send("Input.insertText", { text: path });
		const observed = await this.evalRaw<string>(`
			(() => {
				const visible = Array.from(
					document.querySelectorAll(".kaos-handoff-recovery-export-path")
				).filter((candidate) =>
				...
				if (visible.length !== 1 || !(visible[0] instanceof HTMLInputElement)) {
					throw new Error("Recovery export path input changed identity");
				}
				return visible[0].value;
			})()
		`);
		if (observed !== path) throw new Error(`Recovery export path mismatch: ${observed}`);
	}
	...
	async waitForVisibleText(
		...
		let last = "";
		while (Date.now() - startedAt < timeoutMs) {
			const snapshot = await this.evalRaw<{ count: number; text: string }>(`
				(() => {
					const visible = Array.from(document.querySelectorAll(${JSON.stringify(selector)}))
						.filter((candidate) => candidate instanceof HTMLElement && candidate.getClientRects().length > 0);
					return {
						count: visible.length,
						text: visible.length === 1 ? visible[0].textContent ?? "" : "",
					};
				})()
			`);
			last = snapshot.text;
			if (snapshot.count === 1 && snapshot.text.includes(expectedSubstring)) return snapshot.text;
			await new Promise((resolve) => setTimeout(resolve, 50));
		}
		throw new Error(
			`visible text timed out: ${selector}/${expectedSubstring}; last=${last}`,
		);
	}
