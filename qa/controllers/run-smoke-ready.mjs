 ...
 *   3. Verifies window.__KAOS_DEBUG__ is mounted
 *   4. Verifies window.__KAOS_QA__ is mounted
 *   5. Verifies QA product build is loaded (getEngineControlPort callable)
 *   6. Verifies harness plugin is loaded (kaos-qa-harness plugin found)
...
async function main() {
	...
	try {
		...
		while (Date.now() - start < QA_TIMEOUT_MS) {
			try {
				qaReady = await client.eval(`!!(window.__KAOS_DEBUG__ && window.__KAOS_QA__)`, 5_000);
				if (qaReady) break;
			} catch {
		...
		// ----------------------------------------------------------------
		const debugExists = await client.eval(`typeof window.__KAOS_DEBUG__ !== "undefined" && window.__KAOS_DEBUG__ !== null`);
		require("window.__KAOS_DEBUG__ exists", debugExists);

		const qaExists = await client.eval(`typeof window.__KAOS_QA__ !== "undefined" && window.__KAOS_QA__ !== null`);
		require("window.__KAOS_QA__ exists", qaExists);
		...
		const harnessLoaded = await client.eval(
			`app?.plugins?.plugins?.["kaos-qa-harness"] != null`,
		);
		require("Harness plugin loaded (kaos-qa-harness)", harnessLoaded);
		...
		// ----------------------------------------------------------------
		try {
			await client.eval(`(async () => { await window.__KAOS_QA__?.stopTrace(); })()`, 10_000);
