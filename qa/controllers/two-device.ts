...
async function s10fSetup(
	...
	await creator.evalRaw(`window.__KAOS_QA__?.createFile(${JSON.stringify(scratch)}, ${JSON.stringify(initial)})`);
	await creator.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(15000)`);

	log("Setup: waiting for file on receiver...");
	await receiver.evalRaw(`window.__KAOS_DEBUG__?.waitForFile(${JSON.stringify(scratch)}, 20000)`);
	await receiver.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(10000)`);

	const hashC = await creator.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
	const hashR = await receiver.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
	if (hashC !== hashR) {
	...
	if (openOnBoth) {
		...
		await creator.evalRaw(`window.__KAOS_QA__?.openFile(${JSON.stringify(scratch)})`);
		await receiver.evalRaw(`window.__KAOS_QA__?.openFile(${JSON.stringify(scratch)})`);
		await creator.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 10000)`);
		await receiver.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 10000)`);
...
async function s10fCleanup(
	...
	await a.evalRaw(`window.__KAOS_QA__?.closeFile(${JSON.stringify(scratch)})`).catch(() => {});
	await b.evalRaw(`window.__KAOS_QA__?.closeFile(${JSON.stringify(scratch)})`).catch(() => {});
	await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(scratch)})`).catch(() => {});
}
...
async function s10fConvergence(
	...
	await passive.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(20000)`).catch(() => {});
	...
	const hashA = await active.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
	const hashP = await passive.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
	if (hashA !== hashP) {
 ...
 * All KaosQaDebugApi calls are proxied through evalRaw into the Obsidian renderer.
 */
function makeCdpDeviceHandle(client: AnyObsidianClient, deviceId: string): DeviceHandle {
	const api: KaosQaDebugApi = {
		// Readiness
		isLocalReady: () => false,
		...
		connectProvider: () => {},
		setQaNetworkHold: () => {},
		waitForLocalReady: (t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForLocalReady(${t})`),
		waitForProviderSynced: (t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForProviderSynced(${t})`),
		waitForProviderDisconnected: (t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForProviderDisconnected(${t})`),
		waitForReconciled: (t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForReconciled(${t})`),
		waitForIdle: (t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(${t})`),
		waitForReceiptAfter: (ts, t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForReceiptAfter(${ts}, ${t})`),
		waitForMemoryReceipt: (t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForMemoryReceipt(${t})`),
		waitForFile: (p, t) => client.evalRaw(`window.__KAOS_DEBUG__?.waitForFile(${JSON.stringify(p)}, ${t})`),
		getReceiptSnapshot: () => ({ candidateId: null, capturedAt: null, lastConfirmedCandidateId: null, lastConfirmedAt: null }),
		getDiskHash: (p) => client.evalRaw(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(p)})`),
		getCrdtHash: (p) => client.evalRaw(`window.__KAOS_DEBUG__?.getCrdtHash(${JSON.stringify(p)})`),
		getEditorHash: (p) => client.evalRaw(`window.__KAOS_DEBUG__?.getEditorHash(${JSON.stringify(p)})`),
		...
		startFlightTrace: (m, s) => client.evalRaw(`window.__KAOS_DEBUG__?.startFlightTrace(${JSON.stringify(m)}, ${JSON.stringify(s)})`),
		stopFlightTrace: () => client.evalRaw(`window.__KAOS_DEBUG__?.stopFlightTrace()`),
		exportFlightTrace: (p) => client.evalRaw(`window.__KAOS_DEBUG__?.exportFlightTrace(${JSON.stringify(p)})`),
		forceReconcile: () => client.evalRaw(`window.__KAOS_DEBUG__?.forceReconcile()`),
		forceReconnect: () => { void client.evalRaw(`window.__KAOS_DEBUG__?.forceReconnect()`); },
		__qaOnlyForceCrdtContentUnsafe: (p, c, o) => client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyForceCrdtContentUnsafe(${JSON.stringify(p)}, ${JSON.stringify(c)}, ${JSON.stringify(o)})`),
	...
	let cachedTraceInfo: ReturnType<KaosQaDebugApi["getActiveTraceInfo"]> = null;
	...
	const startPolling = () => {
		...
		pollTimer = setInterval(() => {
			void client.evalRaw<ReturnType<KaosQaDebugApi["getActiveTraceInfo"]>>(
				`window.__KAOS_DEBUG__?.getActiveTraceInfo() ?? null`,
			).then((info) => { if (info) cachedTraceInfo = info; }).catch(() => {});

			void client.evalRaw<ReadonlyArray<WitnessBufferEntry>>(
				`window.__KAOS_DEBUG__?.getWitnessBuffer() ?? []`,
			).then((buf) => { if (buf) cachedBuffer = buf; }).catch(() => {});

			void client.evalRaw<number>(
				`window.__KAOS_DEBUG__?.currentWitnessSeq() ?? 0`,
...
const TWO_DEVICE_SCENARIOS: Record<string, TwoDeviceScenarioFn> = {
	...
	"offline-handoff-create": async (a, b, log) => {
		...
		await b.evalRaw(`window.__KAOS_DEBUG__?.setQaNetworkHold("offline")`);
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForProviderDisconnected(10000)`);
		log("Device B: provider disconnected.");

		// 2. A creates the file and waits for server receipt
		log("Device A: creating file...");
		await a.evalRaw(
			`window.__KAOS_QA__?.createFile(${JSON.stringify(scratch)}, "# Offline Handoff\\n\\nCreated on A while B offline.\\n")`
		);
		...
		await b.evalRaw(`window.__KAOS_DEBUG__?.setQaNetworkHold("online")`);
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(30000)`).catch((e: unknown) => {
		...
		// Cleanup
		await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(scratch)})`).catch(() => {});
		await b.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(scratch)})`).catch(() => {});
	...
	"delete-does-not-resurrect": async (a, b, log) => {
		...
		await a.evalRaw(
			`window.__KAOS_QA__?.createFile(${JSON.stringify(scratch)}, "# S03 Two-Device Delete\\n")`
		);
		const actionTs = Date.now();
		await a.evalRaw(`window.__KAOS_DEBUG__?.waitForReceiptAfter(${actionTs}, 30000)`);
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForFile(${JSON.stringify(scratch)}, 20000)`);
		...
		await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(scratch)})`);
		await a.evalRaw(`window.__KAOS_QA__?.waitForIdle(10000)`);
		log("Device A: file deleted.");

		// Release B's offline hold
		log("Device B: releasing offline hold...");
		await b.evalRaw(`window.__KAOS_DEBUG__?.setQaNetworkHold("online")`);
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(20000)`).catch((e: unknown) => {
			errors.push(`Device B idle wait after reconnect failed: ${String(e)}`);
		});
	...
	"issue-22-passive-open-roundtrip": async (a, b, log) => {
		...
		await a.evalRaw(`window.__KAOS_QA__?.createFile(${JSON.stringify(scratch)}, ${JSON.stringify(INITIAL)})`);
		await a.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(15000)`);

		log("Setup: waiting for file on Device B...");
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForFile(${JSON.stringify(scratch)}, 20000)`);
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(10000)`);

		// Verify both have same content
		const hashA0 = await a.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
		const hashB0 = await b.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
		if (hashA0 !== hashB0) {
		...
		log("Setup: opening file on both...");
		await a.evalRaw(`window.__KAOS_QA__?.openFile(${JSON.stringify(scratch)})`);
		await b.evalRaw(`window.__KAOS_QA__?.openFile(${JSON.stringify(scratch)})`);
		await a.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 10000)`);
		await b.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 10000)`);
		log("Setup: healthy binding on both ✓");
		...
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(20000)`).catch(() => {});
		await new Promise((r) => setTimeout(r, 5000));
		...
		const hashAFinal = await a.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
		const hashBFinal = await b.evalRaw<string | null>(`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`);
	...
	"issue-22-passive-reconnect": async (a, b, log) => {
		...
		for (let i = 1; i <= CYCLES; i++) {
			...
			if (!disconnected) {
				...
				await b.evalRaw(`window.__KAOS_DEBUG__?.setQaNetworkHold("online")`).catch(() => {});
				await new Promise((r) => setTimeout(r, 2000));
				continue;
			}
			...
			await b.evalRaw(`window.__KAOS_QA__?.waitForDiskCrdtConverge(${JSON.stringify(scratch)}, 15000)`).catch(() => {});
	...
	//
	// Two sub-scenarios covering the "I turned KAOS off, edited, turned
	// it back on" reporter path (Issue #22-B):
	//
	//   s10e-1 (issue-22-disable-reenable-local-only):
	//     B edits while KAOS is disabled. A makes NO changes while B is
	...
	//     B edits while KAOS is disabled. A ALSO edits through KAOS while
	...
	"issue-22-disable-reenable-local-only": async (a, b, log) => {
		...
		const EDIT_WHILE_DISABLED = "\nEDITED WHILE KAOS WAS DISABLED. Marker D1S4.\n";
		...
		const preDisableHash = await b.evalRaw<string | null>(
			`window.__KAOS_DEBUG__?.getDiskHash(${JSON.stringify(scratch)})`,
		);
		log(`Pre-disable B disk hash: ${preDisableHash?.slice(0, 12)}`);

		// ── Disable KAOS on B ────────────────────────────────────────────

		log("Action: disabling KAOS plugin on Device B...");
		await b.evalRaw(`app.plugins.disablePlugin("kaos")`);
		await new Promise((r) => setTimeout(r, 3000));

		const kaosUnloaded = await b.evalRaw<boolean>(`!app.plugins?.plugins?.kaos`);
		if (!kaosUnloaded) {
			errors.push("B: KAOS plugin instance still present after disablePlugin");
			await b.evalRaw(`app.plugins.enablePlugin("kaos")`).catch(() => {});
			return { passedA: false, passedB: false, errors };
		}
		log("Action: KAOS unloaded on B ✓");

		// ── A does NOT edit (this is the local-only scenario) ────────────

		log("Note: A makes no changes while B has KAOS disabled (local-only scenario).");
		await new Promise((r) => setTimeout(r, 2000)); // small idle window

		// ── B edits while KAOS is disabled ───────────────────────────────

		log("Action: editing file on B while KAOS is disabled...");
		await b.evalRaw(`
			(async () => {
				const f = app.vault.getAbstractFileByPath(${JSON.stringify(scratch)});
		...
		if (!editedContent.includes(DISABLE_MARKER)) {
			errors.push("B: edit did not land on disk while KAOS was disabled");
			await b.evalRaw(`app.plugins.enablePlugin("kaos")`).catch(() => {});
			return { passedA: false, passedB: false, errors };
		}
		log("Action: edit landed on B disk while KAOS disabled ✓");

		// ── Re-enable KAOS on B ──────────────────────────────────────────

		log("Action: re-enabling KAOS plugin on Device B...");
		...
		const qaReady = await b.evalRaw<boolean>(`
			(async () => {
				const deadline = Date.now() + 30000;
				while (Date.now() < deadline) {
					const kaos = app.plugins?.plugins?.kaos;
					const debug = window.__KAOS_DEBUG__;
					if (kaos && debug && debug.isLocalReady()) return true;
		...
		`);
		if (!qaReady) {
			errors.push("B: KAOS did not re-initialize within 30s after enablePlugin");
			return { passedA: false, passedB: false, errors };
		}
		log("Action: KAOS re-initialized on B ✓");
		...
		if (conflictPath) {
			...
			// Cleanup artifact
			await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(conflictPath)})`).catch(() => {});
			await b.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(conflictPath)})`).catch(() => {});
		} else {
			log("Assert: no spurious conflict artifact created ✓");
		}

		// Assert: B's edit reached A (proves disk→CRDT import worked)
		await a.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(15000)`).catch(() => {});
		await new Promise((r) => setTimeout(r, 5000));
	...
	// s10e-2: B edits while KAOS disabled, A ALSO edits through KAOS.
	...
	"issue-22-disable-reenable-concurrent": async (a, b, log) => {
		...
		// Assert: KAOS is actually unloaded (plugin instance destroyed)
		...
		log("Action: KAOS unloaded on B (plugin instance gone) ✓");

		// ── Assert: __KAOS_DEBUG__ removed on unload ──────────────────────
		...
		const debugGoneAfterUnload = await b.evalRaw<boolean>(
			`typeof window.__KAOS_DEBUG__ === "undefined"`,
		);
		if (!debugGoneAfterUnload) {
			errors.push(
				"B: window.__KAOS_DEBUG__ still exists after KAOS unload. " +
				"Stale global not cleaned up in onunload().",
			);
		} else {
			log("Assert: __KAOS_DEBUG__ removed after unload ✓");
		}

		// ── Edit on B while KAOS is disabled ─────────────────────────────
		// Use raw Obsidian vault API -- no KAOS involved
		...
		// Meanwhile, A continues editing through KAOS (A is still online)
		log("Action: A continues editing while B has KAOS disabled...");
		await a.evalRaw(`window.__KAOS_QA__?.openFile(${JSON.stringify(scratch)})`);
		await a.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 10000)`);
		...
		// Wait for KAOS to fully initialize
		// Poll until KAOS plugin is loaded and debug API is functional
		const qaReady = await b.evalRaw<boolean>(`
			(async () => {
				const deadline = Date.now() + 30000;
				while (Date.now() < deadline) {
		...
		log("Action: KAOS re-initialized on B (APIs available) ✓");

		// Assert: fresh __KAOS_DEBUG__ installed after re-enable
		const debugFreshAfterReload = await b.evalRaw<boolean>(
			`typeof window.__KAOS_DEBUG__ !== "undefined" && typeof window.__KAOS_DEBUG__?.isLocalReady === "function"`,
		);
		if (!debugFreshAfterReload) {
			errors.push(
				"B: window.__KAOS_DEBUG__ missing or malformed after KAOS re-enable.",
			);
		} else {
			log("Assert: fresh __KAOS_DEBUG__ installed after re-enable ✓");
		}
		...
		if (audit.conflictPath) {
			await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(audit.conflictPath)})`).catch(() => {});
			await b.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(audit.conflictPath)})`).catch(() => {});
		}
		await a.evalRaw(`window.__KAOS_QA__?.closeFile(${JSON.stringify(scratch)})`).catch(() => {});
		await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(scratch)})`).catch(() => {});
	...
	// s10e-7: Baseline must advance after an edit while KAOS is enabled.
	//
	// Flow:
	//   1. Create file, sync to both. Baseline = INITIAL.
	//   2. B edits the file WHILE KAOS IS RUNNING (normal edit through vault API).
	//      KAOS observes it → diskMirror flushes to A → setDiskWriteCallback fires
	//      → baseline advances to AFTER_ENABLED_EDIT.
	//   3. Wait for the edit to propagate and disk index to be saved (waitForIdle).
	//   4. B disables KAOS (teardownSync → flushAllPendingWrites → saveDiskIndex).
	...
	// operation, not just from initial setup. Without this, a long-lived KAOS
	...
	"issue-22-disable-reenable-baseline-advances": async (a, b, log) => {
		...
		// ── Step 2: B edits while KAOS is running ────────────────────────
		// This goes through the normal vault modify path; KAOS observes the
		// disk change, seeded/imported into CRDT, setDiskWriteCallback fires.

		log("Action: B edits file while KAOS is running (baseline must advance)...");
		const enabledEditContent = `\n${ENABLED_EDIT_MARKER}. Edit through running KAOS.\n`;
		await b.evalRaw(`
			(async () => {
				const f = app.vault.getAbstractFileByPath(${JSON.stringify(scratch)});
		...
		// Wait for KAOS to observe and process this edit (disk→CRDT pipeline)
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(15000)`);
		await new Promise((r) => setTimeout(r, 2000));
		...
		log("Action: disabling KAOS on B (baseline should be post-enabled-edit)...");
		await b.evalRaw(`app.plugins.disablePlugin("kaos")`);
		await new Promise((r) => setTimeout(r, 3000));

		const kaosUnloaded = await b.evalRaw<boolean>(`!app.plugins?.plugins?.kaos`);
		if (!kaosUnloaded) {
			errors.push("B: KAOS still present after disablePlugin");
			await b.evalRaw(`app.plugins.enablePlugin("kaos")`).catch(() => {});
			return { passedA: false, passedB: false, errors };
		}
		log("Action: KAOS unloaded ✓");

		// Assert __KAOS_DEBUG__ cleaned up
		const debugGone = await b.evalRaw<boolean>(`typeof window.__KAOS_DEBUG__ === "undefined"`);
		if (!debugGone) {
			errors.push("B: __KAOS_DEBUG__ still exists after unload");
		} else {
			log("Assert: __KAOS_DEBUG__ removed ✓");
		}
		...
		// ── Step 7: Re-enable KAOS on B ───────────────────────────────────

		log("Action: re-enabling KAOS on B...");
		await b.evalRaw(`app.plugins.enablePlugin("kaos")`);
		...
		`);
		if (!qaReady) {
			errors.push("B: KAOS did not re-initialize within 30s");
			return { passedA: false, passedB: false, errors };
		}
		log("Action: KAOS re-initialized ✓");
	...
	//   2. B disables KAOS.
	//   3. B deletes the file from disk while KAOS is unloaded.
	//   4. A does NOT edit the file.
	//   5. B re-enables KAOS. Startup reconcile runs.
	//      baseline = INITIAL (both sides had it)
	//      disk:  file GONE on B
	//      CRDT:  file present (A has it, unchanged)
	//
	//   Expected: The local delete wins (or is preserved).
	//   The key invariant: KAOS must not silently resurrect the file B deleted.
	...
	"issue-22-disable-reenable-local-delete-remote-unchanged": async (a, b, log) => {
		...
		log("Action: disabling KAOS on B...");
		await b.evalRaw(`app.plugins.disablePlugin("kaos")`);
		await new Promise((r) => setTimeout(r, 3000));
		...
		// ── B deletes file while KAOS is unloaded ─────────────────────────

		log("Action: B deletes file while KAOS is disabled...");
		...
		`);
		if (!deleteOk) {
			errors.push("B: could not delete file while KAOS was disabled");
			await b.evalRaw(`app.plugins.enablePlugin("kaos")`).catch(() => {});
			return { passedA: false, passedB: false, errors };
		}
		...
		// KNOWN FAILURE: KAOS currently resurrects the file. When a file is
		...
		if (fileOnA) {
			...
			// KAOS may tombstone rather than propagate an offline delete
	...
	//   1. Create file, sync to both (baseline recorded).
	//   2. B disables KAOS.
	//   3. B deletes the file while KAOS is unloaded.
	//   4. A edits the file through KAOS (remote changes from baseline).
	//   5. B re-enables KAOS. Startup reconcile runs.
	//      baseline = INITIAL
	//      disk:  file GONE on B
	...
	//   The key invariant: KAOS must not silently resurrect OR silently drop
	...
	"issue-22-disable-reenable-local-delete-remote-edits": async (a, b, log) => {
		...
		log("Action: A edits the same file through KAOS (concurrent with B's delete)...");
		...
		// KAOS must handle delete-vs-edit conflict without silently losing
		...
		const conflictPath = await b.evalRaw<string | null>(`
			(function() {
				...
				const vaultFiles = app.vault.getFiles().map(f => f.path);
				return vaultFiles.find(p =>
					p.includes(baseName) && (p.includes("conflict") || p.includes("KAOS")) && p !== ${JSON.stringify(scratch)}
				) ?? null;
			})()
		`);
		...
		if (fileOnB) {
			...
			if (conflictPath) {
				...
				// Depends on KAOS tombstone semantics for this case. Not hard-failing
	...
	"s11a-passive-stale-echo-witness": async (a, b, log) => {
		...
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.stopFlightTrace()`).catch(() => {});
			await client.evalRaw(`window.__KAOS_DEBUG__?.startFlightTrace("qa-safe", ${JSON.stringify(QA_SECRET)})`);
			log(`s11a: trace started on ${label}`);
		}
		await new Promise((r) => setTimeout(r, 800));

		const deviceIdA = await a.evalRaw<string>(`window.__KAOS_DEBUG__?.getDeviceId() ?? "device-a"`);
		const deviceIdB = await b.evalRaw<string>(`window.__KAOS_DEBUG__?.getDeviceId() ?? "device-b"`);
		...
		const triggerPreBurstDirty = async () => {
			...
			await a.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyClearWitnessSuppressionUnsafe?.(${JSON.stringify(scratch)})`);
			await b.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyClearWitnessSuppressionUnsafe?.(${JSON.stringify(scratch)})`);
			await a.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyTriggerWitnessDirtyUnsafe?.(${JSON.stringify(scratch)})`);
			await b.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyTriggerWitnessDirtyUnsafe?.(${JSON.stringify(scratch)})`);
			await new Promise((r) => setTimeout(r, 2500));
			const seqA = await a.evalRaw<number>(`window.__KAOS_DEBUG__?.currentWitnessSeq?.() ?? -1`);
			const seqB = await b.evalRaw<number>(`window.__KAOS_DEBUG__?.currentWitnessSeq?.() ?? -1`);
		...
		const [result] = await Promise.all([
			runS11a({
				...
				performBurst: async (_deviceA, _path) => {
					...
					const anchorSeqA = await a.evalRaw<number>(`window.__KAOS_DEBUG__?.currentWitnessSeq?.() ?? 0`);
					...
					while (performance.now() - waitStart < 10_000) {
						...
						const buf = await a.evalRaw<Array<{ kind: string; seq: number; data: Record<string, unknown> }>>(
							`JSON.parse(JSON.stringify(window.__KAOS_DEBUG__?.getWitnessBuffer?.() ?? []))`
						);
						if (!buf) break;
					...
					if (!finalHashWitnessedByA) {
						...
						if (traceId) {
							const segs = await a.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments(${JSON.stringify(traceId)}) ?? null`);
							if (segs) {
		...
		for (const [client, label, devId] of [[a, "device-a", deviceIdA], [b, "device-b", deviceIdB]] as const) {
			const ndjson = await client.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments(${JSON.stringify(traceId ?? "")}) ?? null`);
			if (ndjson) {
				try {
				...
				const liveEvents = await client.evalRaw<Array<{ kind: string; seq: number; path: string; data: Record<string, unknown> }>>(
					`JSON.parse(JSON.stringify((window.__KAOS_DEBUG__?.getWitnessBuffer?.() ?? []).filter(e => e.path === ${JSON.stringify(scratch)})))`
				);
	...
	"s11b-disable-reenable-witness": async (a, b, log) => {
		...
		log("s11b: Phase 2 -- disabling KAOS on B...");
		await b.evalRaw(`app.plugins.disablePlugin("kaos")`);
		await new Promise((r) => setTimeout(r, 3000));
		const unloaded = await b.evalRaw<boolean>(`!app.plugins?.plugins?.kaos`);
		if (!unloaded) { errors.push("s11b: KAOS did not unload on B"); stopPollingA(); stopPollingB(); return { passedA: false, passedB: false, errors }; }
		log("s11b: KAOS unloaded on B ✓");
		...
		// Re-enable B
		log("s11b: re-enabling KAOS on B...");
		await b.evalRaw(`app.plugins.enablePlugin("kaos")`);
		...
		`);
		if (!ready) { errors.push("s11b: KAOS did not re-initialize on B"); stopPollingA(); stopPollingB(); return { passedA: false, passedB: false, errors }; }
		log("s11b: KAOS re-initialized on B ✓");
		...
		for (const [client, label, devId] of [[a, "device-a", deviceIdA], [b, "device-b", deviceIdB]] as const) {
			...
			if (ndjson) {
				...
				const liveEvents = await client.evalRaw<Array<{ kind: string; seq: number; path: string; data: Record<string, unknown> }>>(
					`JSON.parse(JSON.stringify((window.__KAOS_DEBUG__?.getWitnessBuffer?.() ?? []).filter(e => e.path === ${JSON.stringify(scratch)} || e.path === ${JSON.stringify(conflictArtifactPath)})))`
				);
				segmentSummaries[label] = {
		...
		if (conflictArtifactPath) {
			await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(conflictArtifactPath)})`).catch(() => {});
			await b.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(conflictArtifactPath)})`).catch(() => {});
		}
		await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(scratch)})`).catch(() => {});
	...
	"s12a-local-two-vault": async (a, b, log) => {
		...
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlySetScenarioRunIdUnsafe?.(${JSON.stringify(RUN_ID)}, ${JSON.stringify(SCENARIO_ID)})`);
		...
		const deviceIdA = await a.evalRaw<string>(`window.__KAOS_DEBUG__?.getDeviceId() ?? "device-a"`);
		const deviceIdB = await b.evalRaw<string>(`window.__KAOS_DEBUG__?.getDeviceId() ?? "device-b"`);
		const traceInfoA = await a.evalRaw<{ localTraceId: string; qaTraceSecretHash: string } | null>(`window.__KAOS_DEBUG__?.getActiveTraceInfo() ?? null`);
		const traceInfoB = await b.evalRaw<{ localTraceId: string; qaTraceSecretHash: string } | null>(`window.__KAOS_DEBUG__?.getActiveTraceInfo() ?? null`);
		...
		await a.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyForceCrdtContentUnsafe?.(${JSON.stringify(scratch)}, ${JSON.stringify(INITIAL)}, { originClass: "remote", createIfMissing: true })`);
		await new Promise((r) => setTimeout(r, 3000));
		...
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyAdvanceScenarioStepUnsafe?.(1, "pre-action-baseline")`);
			log(`s12a: step 1 advanced on ${label}`);
		}
		await new Promise((r) => setTimeout(r, 1000));
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyClearWitnessSuppressionUnsafe?.(${JSON.stringify(scratch)})`);
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyTriggerWitnessDirtyUnsafe?.(${JSON.stringify(scratch)})`);
			log(`s12a: step 1 dirty triggered on ${label}`);
		}
		await new Promise((r) => setTimeout(r, 5000));

		// Step 2 -- Device A edits (remote origin so DiskMirror writes to disk)
		await a.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyForceCrdtContentUnsafe?.(${JSON.stringify(scratch)}, ${JSON.stringify(EDIT)}, { originClass: "remote" })`);
		log("s12a: Device A edit applied, waiting for propagation...");
		await new Promise((r) => setTimeout(r, 5000));

		// Step 3 -- post-edit convergence
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyAdvanceScenarioStepUnsafe?.(3, "post-edit-convergence")`);
		...
		const segmentsA = await a.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments?.(${JSON.stringify(traceIdA)}) ?? null`);
		const segmentsB = await b.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments?.(${JSON.stringify(traceIdB)}) ?? null`);
		...
		// Cleanup
		await a.evalRaw(`window.__KAOS_DEBUG__?.stopFlightTrace()`).catch(() => {});
		await b.evalRaw(`window.__KAOS_DEBUG__?.stopFlightTrace()`).catch(() => {});
	 ...
	 * Device A edits the file through the normal KAOS path.
	...
	"s13-editor-open-remote-edit": async (a, b, log) => {
		...
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.stopFlightTrace()`).catch(() => {});
			await client.evalRaw(`window.__KAOS_DEBUG__?.startFlightTrace("qa-safe")`);
			log(`s13: fresh trace started on ${label}`);
		}
		await new Promise((r) => setTimeout(r, 1000));
		for (const [client] of [[a], [b]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlySetScenarioRunIdUnsafe?.(${JSON.stringify(RUN_ID)}, "s13-editor-open-remote-edit")`);
		}
		await new Promise((r) => setTimeout(r, 300));
		const freshBufLenB = await b.evalRaw<number>(`(window.__KAOS_DEBUG__?.getWitnessBuffer() ?? []).length`);
		...
		const seqAnchorB = await b.evalRaw<number>(`window.__KAOS_DEBUG__?.currentWitnessSeq?.() ?? 0`);
		...
		await a.evalRaw(`window.__KAOS_QA__?.createFile(${JSON.stringify(scratch)}, ${JSON.stringify(INITIAL)})`);
		await a.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(15000)`);
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForFile(${JSON.stringify(scratch)}, 30000)`);
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(10000)`);
		// Extra wait for disk writes to complete on both devices
		await new Promise((r) => setTimeout(r, 4000));
		await a.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(5000)`).catch(() => {});
		await b.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(5000)`).catch(() => {});
		...
		const healthBefore = await b.evalRaw<Record<string, unknown>>(`JSON.parse(JSON.stringify(window.__KAOS_DEBUG__?.getEditorBindingHealth(${JSON.stringify(scratch)}) ?? {}))`);
		...
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyAdvanceScenarioStepUnsafe?.(1, "baseline-editor-open")`);
		...
		const segsB1Raw = await b.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments?.(${JSON.stringify(traceIdBForSegs)}) ?? null`);
		...
		// Step 2 -- Device A edits via normal KAOS path (editor insert)
		await a.evalRaw(`window.__KAOS_QA__?.openFile(${JSON.stringify(scratch)})`);
		await a.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 10000)`);
		await new Promise((r) => setTimeout(r, 1000));
		...
		await a.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(8000)`).catch(() => {});
		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyAdvanceScenarioStepUnsafe?.(3, "post-edit-convergence")`);
		...
		const healthAfter = await b.evalRaw<Record<string, unknown>>(`JSON.parse(JSON.stringify(window.__KAOS_DEBUG__?.getEditorBindingHealth(${JSON.stringify(scratch)}) ?? {}))`);
		...
		const segsB2Raw = await b.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments?.(${JSON.stringify(traceIdBForSegs)}) ?? null`);
		...
		const segsA = await a.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments?.(${JSON.stringify(traceInfoA?.localTraceId ?? "")}) ?? null`);
		const segsB = await b.evalRaw<string | null>(`window.__KAOS_DEBUG__?.exportWitnessSegments?.(${JSON.stringify(traceInfoB?.localTraceId ?? "")}) ?? null`);
	...
	"s12a-with-edit": async (a, b, log) => {
		...
		await b.evalRaw(`window.__KAOS_QA__?.openFile(${JSON.stringify(scratch)})`);
		await new Promise((r) => setTimeout(r, 2000));
		await a.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 20000)`).catch(() => {});
		await b.evalRaw(`window.__KAOS_QA__?.waitForCrdtBinding(${JSON.stringify(scratch)}, 20000)`).catch(() => {});
		await new Promise((r) => setTimeout(r, 2000));

		for (const [client, label] of [[a, "A"], [b, "B"]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyAdvanceScenarioStepUnsafe?.(1, "pre-edit-baseline")`);
		}
		await new Promise((r) => setTimeout(r, 500));
		for (const [client] of [[a], [b]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyClearWitnessSuppressionUnsafe?.(${JSON.stringify(scratch)})`);
		...
		const healthB = await b.evalRaw<Record<string, unknown>>(`JSON.parse(JSON.stringify(window.__KAOS_DEBUG__?.getEditorBindingHealth(${JSON.stringify(scratch)}) ?? {}))`);
	 ...
	 * B disabled, A edits via KAOS, B edits disk directly, B re-enables.
	...
	"s12c-conflict": async (a, b, log) => {
		...
		for (const [client] of [[a], [b]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyAdvanceScenarioStepUnsafe?.(1, "baseline-quorum")`);
		}
		await new Promise((r) => setTimeout(r, 500));
		for (const [client] of [[a], [b]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyClearWitnessSuppressionUnsafe?.(${JSON.stringify(scratch)})`);
		...
		if (!bDisabled) errors.push("s12c: B KAOS not disabled");
		log(`s12c: B KAOS disabled: ${bDisabled}`);
		...
		await a.evalRaw(`window.__KAOS_DEBUG__?.waitForIdle(10000)`).catch(() => {});
		await new Promise((r) => setTimeout(r, 3000));
		log("s12c: A inserted REMOTE_FROM_A via KAOS");
		...
		const artifactPath = await b.evalRaw<string | null>(`(function(){const files=app.vault.getMarkdownFiles();const a=files.find(f=>f.path.includes("s12c-conflict")&&f.path.includes("KAOS conflict"));return a?.path??null;})()`);
		...
		const artifactsOnA = await a.evalRaw<string[]>(`app.vault.getMarkdownFiles().filter(f=>f.path.includes("s12c-conflict")&&f.path.includes("KAOS conflict")).map(f=>f.path)`);
		...
		await new Promise((r) => setTimeout(r, 500));
		for (const [client] of [[a], [b]] as const) {
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyClearWitnessSuppressionUnsafe?.(${JSON.stringify(scratch)})`).catch(() => {});
			await client.evalRaw(`window.__KAOS_DEBUG__?.__qaOnlyTriggerWitnessDirtyUnsafe?.(${JSON.stringify(scratch)})`).catch(() => {});
		}
		await new Promise((r) => setTimeout(r, 6000));
		...
		await a.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(scratch)})`).catch(() => {});
		if (artifactPath) await b.evalRaw(`window.__KAOS_QA__?.deleteFile(${JSON.stringify(artifactPath)})`).catch(() => {});
		await a.evalRaw(`window.__KAOS_DEBUG__?.stopFlightTrace()`).catch(() => {});
		await b.evalRaw(`window.__KAOS_DEBUG__?.stopFlightTrace()`).catch(() => {});
