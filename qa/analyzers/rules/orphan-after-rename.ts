...
export function checkOrphanAfterRename(events: FlightEvent[]): AnalyzerFinding[] {
	...
	// of B→C). KAOS collapses chains into a single CRDT rename (A→C), so B never gets
	...
	for (const renameEvent of targetRenames) {
		...
		// KAOS collapses the chain in the CRDT batch, so no crdt.file.renamed
		...
		if (!crdtRenamed) {
			...
			// If oldPath had a crdt.file.created event before the rename fired, KAOS should
