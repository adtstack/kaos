...
export const runVfsTortureTest = async (context: VfsTortureTestContext): Promise<void> => {
	...
	const rootDir = normalizePath(`KAOS QA/vfs-torture-${runId}`);
	const steps: Array<{
		name: string;
	...
	if (failures.length > 0) {
		new Notice(
			`KAOS VFS torture run finished with ${failures.length} failed step(s). Report: ${outPath}`,
			12000,
		);
		context.log(
	...
	new Notice(`KAOS VFS torture run completed. Report: ${outPath}`, 10000);
