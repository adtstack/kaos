/**
 * KAOS QA Debug API — Puppeteer harness (qa/harness/)
 *
 ...
 *   - KaosDebugPort: safe, read-only or non-mutating debug capabilities
 *   - KaosUnsafeQaPort: scenario control and unsafe mutation
 */
...
export interface KaosQaDebugApi {
	// Readiness
	 ...
	 * qaTraceSecretHash is SHA-256("kaos-qa-trace-secret:" + qaTraceSecret).
...
// These ensure KaosQaDebugApi is assignable to both KaosDebugPort and
// KaosUnsafeQaPort. If any method signature drifts, this file will fail
...
import type { KaosUnsafeQaPort } from "./ports/kaosUnsafeQaPort";

// Full assignability checks — not just method names, but full signatures.
// If KaosQaDebugApi is not assignable to a port, the compiler will show
// exactly which methods have incompatible signatures.
type _AssertDebugPortAssignable = KaosQaDebugApi extends KaosDebugPort ? true : never;
type _AssertUnsafePortAssignable = KaosQaDebugApi extends KaosUnsafeQaPort ? true : never;
...
export function buildQaDebugApi(plugin: PluginHandle): KaosQaDebugApi {
	const { app } = plugin;
	...
	const api: KaosQaDebugApi = {
		...
		async __qaOnlyForceCrdtContentUnsafe(
			...
			if (!ytext) {
				const ensureResult = vaultSync.ensureFile(path, content, "qa");
				switch (ensureResult.kind) {
					case "created":
					case "existing":
						ytext = ensureResult.ytext;
						break;
					case "replan":
					case "blocked":
						return { beforeHash: null, afterHash: null, fileExisted };
					default:
		...
		__qaOnlyAdvanceScenarioStepUnsafe(stepIndex: number, label?: string): void {
			...
			if (!state.scenarioRunId) {
				new Notice("set scenarioRunId via 'KAOS QA: Set scenario run ID' first");
				return;
			}
