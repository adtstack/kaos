/**
 * KAOS QA Harness — Obsidian plugin entry point.
 *
 * This plugin:
 *   1. Registers window.__KAOS_QA__ with the full QaConsoleApi
 *   2. Mounts window.__KAOS_DEBUG__ by assembling a PluginHandle from the
 *      product plugin (kaos) + its TelemetryRuntimeHandle.  This is the
 ...
 *
 * Install this plugin alongside KAOS in a QA vault with qaDebugMode enabled.
...
export default class KaosQaHarnessPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		console.log(
			"[KAOS QA] Harness loaded. " +
			`${this.scenarioRegistry.size} scenarios registered. ` +
			"window.__KAOS_QA__ available. " +
			"Type KAOS_QA.help() in console.",
		);
		...
		this.addCommand({
			...
			callback: () => {
				(window as unknown as Record<string, unknown>).__KAOS_QA__ &&
		...
		this.addCommand({
			...
			callback: () => {
				...
				console.log("[KAOS QA] Scenarios:", ids);
			},
		});
		...
		this.addCommand({
			...
			callback: async () => {
				...
				console.log("[KAOS QA] Vault manifest:", JSON.stringify(m, null, 2));
	 ...
	 * result at window.__KAOS_DEBUG__.
	 *
	 * Guards (all must pass before mounting):
	 *   - product plugin "kaos" is loaded
	 ...
	 *
	 * If guards fail, a loud error is logged and no __KAOS_DEBUG__ is mounted.
	 ...
	 * before "kaos-qa-harness" in the vault's community-plugins.json.
	 *
	...
	private mountKaosDebugApi(): void {
		...
		if (!product) {
			console.error(
				...
				"window.__KAOS_DEBUG__ will NOT be mounted. " +
				"Ensure 'kaos' appears before 'kaos-qa-harness' in community-plugins.json.",
			);
		...
		if (typeof (product as any).getEngineControlPort !== "function") {
			console.error(
				"[KAOS QA] FATAL: product plugin is missing getEngineControlPort. " +
		...
		if (!lab) {
			console.error(
				"[KAOS QA] FATAL: product.lab (TelemetryRuntimeHandle) is null. " +
		...
		if (typeof (lab as any).getDeviceWitnessTracker !== "function") {
			console.error(
				"[KAOS QA] FATAL: lab.getDeviceWitnessTracker is missing. " +
