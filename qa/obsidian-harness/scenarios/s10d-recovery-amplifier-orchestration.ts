...
export const s10dRecoveryAmplifierOrchestration: QaScenario = {
	...
	async run(ctx: QaContext): Promise<void> {
		...
		const diskHashPre = await ctx.kaos.getDiskHash(SCRATCH);
		const crdtHashPre = await ctx.kaos.getCrdtHash(SCRATCH);
		...
		//    (because diskMirror didn't write it). KAOS will see:
		...
		const diskHashPost = await ctx.kaos.getDiskHash(SCRATCH);
		const crdtHashPost = await ctx.kaos.getCrdtHash(SCRATCH);
		const editorHashPost = await ctx.kaos.getEditorHash(SCRATCH);
		...
		while (Date.now() - soakStart < SOAK_DURATION) {
			...
			const diskNow = await ctx.kaos.getDiskHash(SCRATCH);
			const crdtNow = await ctx.kaos.getCrdtHash(SCRATCH);
