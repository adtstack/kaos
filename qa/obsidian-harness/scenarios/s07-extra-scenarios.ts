 ...
 *   Extends S07h to stress-test KAOS's watcher/coalescing path at a
...
export const s07iFolderThenFile: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		// 1. Create folder tree (KAOS does not sync empty folders by design).
		...
		const diskIndex = await ctx.kaos.getDiskHash(notePath);
		const crdtIndex = await ctx.kaos.getCrdtHash(notePath);
		...
		const diskInner = await ctx.kaos.getDiskHash(innerNotePath);
		const crdtInner = await ctx.kaos.getCrdtHash(innerNotePath);
		...
		const ghostCrdt = await ctx.kaos.getCrdtHash(ghostPath);
		if (ghostCrdt) {
...
export const s07jAttachmentRefBeforeBlob: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const diskHash = await ctx.kaos.getDiskHash(notePath);
		const crdtHash = await ctx.kaos.getCrdtHash(notePath);
...
export const s07kBlobArrivesAfterReference: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const diskBeforeBlob = await ctx.kaos.getDiskHash(notePath);
		const crdtBeforeBlob = await ctx.kaos.getCrdtHash(notePath);
		...
		const blobDisk = await ctx.kaos.getDiskHash(blobPath);
		console.log("[S07k] blob created", { blobPath, blobDisk });
		// Give KAOS time to process the blob create event.
		await ctx.sleep(2000);
		await ctx.waitForIdle(10000);

		// 4. POSTCONDITION: markdown CRDT hash must be unchanged after blob arrival.
		const diskAfterBlob = await ctx.kaos.getDiskHash(notePath);
		const crdtAfterBlob = await ctx.kaos.getCrdtHash(notePath);
		...
		const blobCrdt = await ctx.kaos.getCrdtHash(blobPath);
