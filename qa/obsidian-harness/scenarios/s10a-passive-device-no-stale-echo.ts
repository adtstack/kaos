 ...
 * for a file that is open in the editor but idle. Prove that KAOS classifies
...
export const s10aPassiveDeviceNoStaleEcho: QaScenario = {
	...
	async run(ctx: QaContext): Promise<void> {
		...
		const editorHash = await ctx.kaos.getEditorHash(SCRATCH);
		const crdtHash = await ctx.kaos.getCrdtHash(SCRATCH);
		...
		const result1 = await ctx.kaos.__qaOnlyForceCrdtContentUnsafe(
			SCRATCH,
			REMOTE_DELETION,
			{ originClass: "remote" },
		);
		...
		const diskAfterDel = await ctx.kaos.getDiskHash(SCRATCH);
		const crdtAfterDel = await ctx.kaos.getCrdtHash(SCRATCH);
		const editorAfterDel = await ctx.kaos.getEditorHash(SCRATCH);
		...
		const result2 = await ctx.kaos.__qaOnlyForceCrdtContentUnsafe(
			SCRATCH,
			REMOTE_ADDITION,
			{ originClass: "remote" },
		);
		...
		const crdtAfterAdd = await ctx.kaos.getCrdtHash(SCRATCH);
		...
		const stableHash = await ctx.kaos.getCrdtHash(SCRATCH);
		for (let i = 0; i < 6; i++) {
			await ctx.sleep(1000);
			const h = await ctx.kaos.getCrdtHash(SCRATCH);
			if (h !== stableHash) {
				throw new Error(
