...
export const s09aRenameIntoExcluded: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const crdtV1 = await ctx.kaos.getCrdtHash(syncPath);
		...
		const crdtAtSrc = await ctx.kaos.getCrdtHash(syncPath);
		if (crdtAtSrc) {
			throw new Error(
		...
		const crdtAtDest = await ctx.kaos.getCrdtHash(excludedDest);
		if (crdtAtDest) {
			throw new Error(
...
export const s09bRenameFromExcluded: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const crdtAtSrc = await ctx.kaos.getCrdtHash(excludedSrc);
		if (crdtAtSrc) {
			throw new Error(
		...
		const diskDest = await ctx.kaos.getDiskHash(syncDest);
		if (!diskDest) {
		...
		const crdtDest = await ctx.kaos.getCrdtHash(syncDest);
...
export const s09cRenameToVacatedPath: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const diskB = await ctx.kaos.getDiskHash(pathB);
