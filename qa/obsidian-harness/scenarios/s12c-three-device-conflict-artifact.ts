 ...
 *
 * STEP 4 — Disable KAOS on Device B (iPad)
 *   Device B: disable KAOS plugin (Settings → Community plugins → KAOS → disable).
 *
 ...
 *
 * STEP 6 — Local edit on Device B (while KAOS disabled)
 *   Device B: edit "QA-scratch/s12c-witness.md" directly (e.g. via Files app or another editor).
 *   Content: "S12C-LOCAL"
 *
 * STEP 7 — Re-enable KAOS on Device B
 *   Device B: enable KAOS plugin.
 *   Wait for re-sync (30 seconds).
 *
 * STEP 8 — Verify conflict artifact on Device B
 *   Device B: check that a conflict artifact file exists at the standard KAOS conflict path.
