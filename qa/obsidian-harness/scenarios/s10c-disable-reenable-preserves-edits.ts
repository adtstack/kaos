 ...
 *
 * Purpose: Prove that edits made while KAOS is disconnected are preserved
 * when the provider reconnects, and are NOT overwritten by stale CRDT state
 * from the server.
 *
 * This targets the user report: "I tried turning off the KAOS plugin, then
...
export const s10cDisableReenablePreservesEdits: QaScenario = {
	...
	async run(ctx: QaContext): Promise<void> {
		...
		ctx.kaos.setQaNetworkHold("offline");
		...
		ctx.kaos.setQaNetworkHold("online");
