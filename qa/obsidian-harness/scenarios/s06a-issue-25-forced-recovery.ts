 ...
 *        - Vault modify event fires → KAOS calls handleBoundFileSyncGap.
...
export const s06aIssue25ForcedRecoveryCrdtOnly: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		// 1. Create file. KAOS seeds CRDT from the disk.create event.
		...
		const diskHash0 = await ctx.kaos.getDiskHash(path);
		const crdtHash0 = await ctx.kaos.getCrdtHash(path);
		const editorHash0 = await ctx.kaos.getEditorHash(path);
		...
		await ctx.sleep(200);
		await ctx.kaos.__qaOnlyForceCrdtContentUnsafe(path, fixture, {
			originClass: "local",
...
export const s06aIssue25ForcedRecoveryLocalOnly: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		paused = await ctx.kaos.pauseEditorPropagation(path);
		if (!paused) {
		...
		const diskAfterEdit = await ctx.kaos.getDiskHash(path);
		const crdtAfterEdit = await ctx.kaos.getCrdtHash(path);
		const editorAfterEdit = await ctx.kaos.getEditorHash(path);
