...
export const s10gSuppressionDelayRace: QaScenario = {
	...
	async run(ctx: QaContext): Promise<void> {
		...
		const injectResult = await ctx.kaos.forceCrdtContent(
			SCRATCH,
			REMOTE_EDIT,
			{ originClass: "remote" },
		);
		...
		const crdtBefore = await ctx.kaos.getCrdtHash(SCRATCH);
		...
		await ctx.kaos.ingestDiskFileNow(SCRATCH, "modify");
		...
		const crdtAfter = await ctx.kaos.getCrdtHash(SCRATCH);
