...
export const s07gRenameAfterCreate: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		// KAOS processes disk events asynchronously. waitForCrdtFile polls
		...
		const diskBefore = await ctx.kaos.getDiskHash(tmpPath);
		const crdtBefore = await ctx.kaos.getCrdtHash(tmpPath);
		...
		const diskFinal = await ctx.kaos.getDiskHash(finalPath);
		const crdtFinal = await ctx.kaos.getCrdtHash(finalPath);
		...
		const crdtOld = await ctx.kaos.getCrdtHash(tmpPath);
		if (crdtOld) {
...
//   2. Plugin immediately calls vault.rename() before KAOS has processed
...
//
// If KAOS silently drops the rename:
...
export const s07gRenameBeforeCrdtRegistration: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		// On fast hardware KAOS registers files synchronously within the vault
		...
		const crdtAtZero = await ctx.kaos.getCrdtHash(tmpPath);
		const raceHit = crdtAtZero === null;
		console.log("[S07g-2] pre-rename CRDT state", { crdtAtZero, raceHit });

		if (!raceHit) {
			// CRDT already registered. This means KAOS processes creates
		...
		// 3. Now wait for the dust to settle. Give KAOS enough time to
		...
		const refDisk = await ctx.kaos.getDiskHash(refPath);
...
export const s07gRenameToTombstonedPath: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const crdtAfterDelete = await ctx.kaos.getCrdtHash(tombstonedPath);
		if (crdtAfterDelete) {
		...
		const diskBeforeRename = await ctx.kaos.getDiskHash(tmpPath);
		const crdtBeforeRename = await ctx.kaos.getCrdtHash(tmpPath);
		...
		// 4. POSTCONDITION.
		const diskFinal = await ctx.kaos.getDiskHash(tombstonedPath);
		const crdtFinal = await ctx.kaos.getCrdtHash(tombstonedPath);
...
export const s07gRenameChain: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const diskAtA = await ctx.kaos.getDiskHash(pathA);
		const crdtAtA = await ctx.kaos.getCrdtHash(pathA);
		...
		const diskC = await ctx.kaos.getDiskHash(pathC);
		const crdtC = await ctx.kaos.getCrdtHash(pathC);
		...
		const crdtA = await ctx.kaos.getCrdtHash(pathA);
		const crdtB = await ctx.kaos.getCrdtHash(pathB);
...
export const s07gModifyThenRenameChain: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const crdtV1 = await ctx.kaos.getCrdtHash(pathA);
...
export const s07gModifyThenRename: QaScenario = {
	...
	async run(ctx): Promise<void> {
		...
		const crdtV1 = await ctx.kaos.getCrdtHash(tmpPath);
