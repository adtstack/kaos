 ...
 *
 * What it proves:
 *   - window.__KAOS_DEBUG__ is mounted (harness reached the product plugin)
 *   - window.__KAOS_QA__ is mounted (harness plugin itself is running)
...
export const s00SmokeTraceExport: QaScenario = {
	...
	async run(ctx: QaContext): Promise<void> {
		...
		// of the provider state. It proves __KAOS_DEBUG__ is callable and that
		// the internal provider state machine is reachable.
		const connectionState = ctx.kaos.getConnectionState();
