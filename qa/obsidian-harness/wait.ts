...
export function waitForIdle(
	...
	return kaos.waitForIdle(timeoutMs);
}
...
export function waitForMemoryReceipt(
	...
	return kaos.waitForMemoryReceipt(timeoutMs);
}
...
export function waitForFile(
	...
	return kaos.waitForFile(path, timeoutMs);
}
...
export function waitForDiskCrdtConverge(
	...
	return waitForCondition(
		async () => {
			const disk = await kaos.getDiskHash(path);
			const crdt = await kaos.getCrdtHash(path);
...
export function waitForCrdtFile(
	...
	return waitForCondition(
		async () => (await kaos.getCrdtHash(path)) !== null,
		timeoutMs,
...
export function waitForFileContent(
	...
	return waitForCondition(
		async () => {
			const actual = await kaos.getDiskHash(path);
...
export function waitForCrdtBinding(
	...
	return waitForCondition(
		() => {
			const h = kaos.getEditorBindingHealth(path);
			return h.healthy;
		},
		timeoutMs,
...
export function waitForActiveMarkdownLeaf(
	...
	return waitForCondition(
		() => {
			...
			const activePaths = kaos.getActiveMarkdownPaths();
...
export function witnessDeviceSettled(
	...
	return kaos.witnessDeviceSettled(path, options);
}
