...
export class PairDeviceModal extends Modal {
	...
	onOpen(): void {
		...
		const qrWrap = contentEl.createDiv({ cls: "kaos-pair-device-qr-wrap" });

		const loadingEl = qrWrap.createEl("div", {
			text: "Generating setup code...",
			cls: "kaos-pair-device-loading",
		});
		...
		const mobileInput = manualBody.createEl("textarea", { cls: "kaos-settings-modal-textarea" });
		...
		const deepInput = manualBody.createEl("textarea", { cls: "kaos-settings-modal-textarea" });
