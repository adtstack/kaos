...
export class RecoveryKitModal extends Modal {
	...
	onOpen(): void {
		...
		contentEl.empty();
		contentEl.addClass("kaos-recovery-kit-modal");

		contentEl.createEl("h3", { text: "Backup connection details" });

		const warning = contentEl.createDiv({ cls: "callout kaos-settings-callout" });
		...
		const textArea = contentEl.createEl("textarea", { cls: "kaos-settings-modal-textarea" });
