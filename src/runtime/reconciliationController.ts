...
/**
 * Bound consecutive disk events from one host/plugin write burst. Lineage
 * equality is still mandatory; time alone never authorizes coalescing.
 */
const EXTERNAL_DISK_TRANSIENT_REVISION_WINDOW_MS = 3000;
...
export class ReconciliationController {
	...
	noteInterceptedExternalDiskMutation(candidate: InterceptedExternalDiskMutation): void {
		...
		if (current && current.sequence > candidate.sequence) {
			if (current.content !== candidate.content) {
				if (this.canCoalesceTransientExternalDiskRevision(candidate, current)) {
					this.traceTransientExternalDiskRevisionCoalesced(candidate, current);
				} else {
		...
		if (
			...
			if (this.canCoalesceTransientExternalDiskRevision(current, candidate)) {
				this.traceTransientExternalDiskRevisionCoalesced(current, candidate);
			} else {
	...
	private canCoalesceTransientExternalDiskRevision(
		...
		if (
			...
			!Number.isFinite(older.observedAt) ||
			!Number.isFinite(newer.observedAt)
		) {
			return false;
		}
		const elapsedMs = newer.observedAt - older.observedAt;
		if (
			elapsedMs < 0 ||
			elapsedMs > EXTERNAL_DISK_TRANSIENT_REVISION_WINDOW_MS
		) {
			return false;
		}
		...
		return this.sameExternalEditorAuthorityLineage(
			olderLineage,
			newerLineage,
			true,
		);
	...
	private traceTransientExternalDiskRevisionCoalesced(
		older: InterceptedExternalDiskMutation,
		newer: InterceptedExternalDiskMutation,
	): void {
		this.deps.trace("reconcile", "open-external-transient-revision-coalesced", {
			path: newer.path,
			olderSequence: older.sequence,
			newerSequence: newer.sequence,
			elapsedMs: newer.observedAt - older.observedAt,
