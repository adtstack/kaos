...
// Deployment repositories are detached copies, so an already-committed workflow
// cannot be upgraded remotely. Keep this versioned whenever its bootstrap
// contract changes; v3 can recover deployments that predate the updater script.
...
export function buildGithubOpsBootstrapWorkflowYaml(): string {
	return [
		...
		"          git commit -m \"kaos(server): update to ${VERSION}\"",
		"          git push",
...
export class CapabilityUpdateService {
	...
	enforceCompatibilityGuard(reason: string): boolean {
		...
		console.warn(`[kaos] Compatibility guard (${reason}): ${blockReason}`);
		if (firstBlock) {
	...
	private async handleCapabilityChange(
		...
		if (gainedR2) {
			const message = this.deps.getSettings().enableAttachmentSync
			...
			const message = "KAOS: object storage is unavailable. Deploy/update the Worker with the KAOS_BUCKET R2 binding; attachment transfers and snapshots are paused.";
	...
	private maybeShowUpdateNotices(reason: string): void {
		...
		if (updateState.serverUpdateAvailable && updateState.latestServerVersion) {
			if (this.lastServerUpdateNoticeVersion !== updateState.latestServerVersion) {
				const consoleMessage = `KAOS: server update available (${updateState.serverVersion ?? "unknown"} -> ${updateState.latestServerVersion}). Deploy/update the Worker.`;
				console.warn(`[kaos] ${consoleMessage}`);
		...
		if (updateState.pluginUpdateRecommended && updateState.latestPluginVersion) {
			if (this.lastPluginUpdateNoticeVersion !== updateState.latestPluginVersion) {
				console.warn(
					`[kaos] KAOS: plugin update recommended (${this.deps.pluginVersion} -> ${updateState.latestPluginVersion}).`,
				);
				new Notice(
