...
export class FlightRecorder {
	...
	record(event: FlightEventInput, options?: { reservedSeq?: number }): void {
		...
		if (event.data) {
			...
			if (forbidden) {
				console.error(
					`[kaos/flight] Absolutely forbidden key "${forbidden}" in event data (kind=${String(event.kind)}) — dropping event`,
				);
		...
		if (this.safeToShare && event.data) {
			...
			if (leaked) {
				console.error(
					`[kaos/flight] Sensitive key "${leaked}" found in event data (kind=${String(event.kind)}) — dropping event, emitting redaction.failure`,
				);
				this.redactionDroppedCount++;
				this.redactionDroppedByKind.set(String(event.kind), (this.redactionDroppedByKind.get(String(event.kind)) ?? 0) + 1);
