 ...
 *   6. server/src/version.ts SERVER_MIN_SCHEMA_VERSION is a valid lower bound
 */
...
// 5 & 6. Server schema range must include the plugin schema version and cap at it.
if (!existsSync("server/src/version.ts")) {
	...
	if (!minMatch || !maxMatch) {
		...
		if (min > max) {
			fail(`server/src/version.ts schema range is invalid: min ${min} > max ${max}`);
		}
		if (min > EXPECTED_SCHEMA_VERSION) {
			fail(`server/src/version.ts SERVER_MIN_SCHEMA_VERSION = ${min}, but plugin schema is ${EXPECTED_SCHEMA_VERSION}`);
		} else if (min < 1) {
			fail(`server/src/version.ts SERVER_MIN_SCHEMA_VERSION = ${min}, expected a positive schema version`);
		} else {
		...
		if (min <= EXPECTED_SCHEMA_VERSION && max === EXPECTED_SCHEMA_VERSION && min <= max) {
			pass(`server schema range v${min}-v${max} includes plugin schema v${EXPECTED_SCHEMA_VERSION}`);
		}
	}
}
...
if (failures > 0) {
	...
	console.error("  and server/src/version.ts max must match while min remains a valid compatibility floor.");
	process.exit(1);
} else {
