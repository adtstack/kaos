...
const DEFAULT_TOKEN_FILE = "/etc/kaos/sync-token";
...
async function main() {
	...
	const tokenSecret = await readSecretForRedaction(raw);
...
async function runInstallPhase(raw, logDir, redactor, completed) {
	...
	const tokenInputFile = raw["token-file"] ? resolve(raw["token-file"]) : null;
	if (raw["skip-bootstrap"] !== "true") {
		...
		if (!tokenInputFile) throw new Error("--token-file is required during install phase unless --skip-bootstrap is used.");
	}
	...
	if (raw["skip-bootstrap"] !== "true") {
		const token = await readFile(tokenInputFile, "utf8");
		await runStep({
			name: "bootstrap",
	...
	await runStep({
		...
		outfile: join(logDir, "04-secret-permissions.txt"),
		...
		args: [
			...
			raw["token-destination-file"] ?? DEFAULT_TOKEN_FILE,
		],
		redactor,
		completed,
	});
	...
	await runStep({
		name: "install-postflight",
		outfile: join(logDir, "05-install-postflight.json"),
		command: commandWithSudo(raw, process.execPath)[0],
	...
	await runStep({
		...
		outfile: join(logDir, "06-install-metadata.json"),
...
async function runPostRebootPhase(raw, logDir, redactor, completed) {
	...
	await runStep({
		...
		args: [
			...
			"--token-file",
			resolve(raw["token-destination-file"] ?? DEFAULT_TOKEN_FILE),
			"--work-dir",
			resolve(raw["smoke-work-dir"] ?? DEFAULT_SMOKE_WORK_DIR),
			"--require-lock",
		],
		redactor,
		completed,
...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		for (const flag of ["no-sudo", "no-smoke-user", "skip-bootstrap"]) {
			if (arg === `--${flag}`) {
				out[flag] = "true";
				continue;
			}
...
async function readSecretForRedaction(raw) {
	if (raw["token-file"]) {
		try {
	...
	const command = commandWithSudo(raw, raw["cat-command"] ?? "cat");
	const result = spawnSync(command[0], [...command.slice(1), resolve(raw["token-destination-file"] ?? DEFAULT_TOKEN_FILE)], {
		encoding: "utf8",
		timeout: 15_000,
		input: "",
	});
...
Options:
  --phase <name>                 install, update, or post-reboot. Defaults to install.
  ...
  --token-file <path>            Local token source for bootstrap stdin and redaction.
  ...
  --cat-command <path>           cat command override. Defaults to cat.
