...
console.log("\n--- main.ts retirement wiring ---");
{
	...
	assert(
		retireBody.indexOf("legacyMissingBlobPaths.add(") > retireBody.indexOf("retire-authority-moved")
			&& retireBody.indexOf("normalizePath(") < retireBody.indexOf("legacyMissingBlobPaths.add("),
		"authority-moved retirement registers the normalized path in the legacy quarantine registry",
	);
	assert(
		retireBody.includes("settlementChanged || legacyQuarantineChanged"),
		"the legacy quarantine registry change persists with the settled-ref snapshot",
	);
	assert(
		retireBody.indexOf("enqueueBlobSettledRefPersistence()") >= 0
			&& retireBody.indexOf("enqueueBlobSettledRefPersistence()")
				< retireBody.indexOf("enqueuePendingBlobIntentPersistence()"),
		"the download-gate snapshot persists before the journal fence removal",
	);
	assert(
	...
	assert(
		dismissBody.indexOf("commitAttemptId === undefined") >= 0
			&& dismissBody.indexOf("commitAttemptId === undefined")
				< dismissBody.indexOf("retireStuckBlobEpisode("),
		"dismissal retires only intents that own a stuck episode",
	);
}
