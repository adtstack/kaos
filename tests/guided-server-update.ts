...
assert.match(
	parsedBootstrapUrl.searchParams.get("value") ?? "",
	/immutable bootstrap[\s\S]*?git add -A -- ':!\.github\/workflows\/kaos-ops-v3\.yml'/,
	"bootstrap workflow keeps its versioned control plane immutable while updating release-owned server files",
);
