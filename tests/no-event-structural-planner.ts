...
console.log("\n--- No-event structural planner: matching prior baseline safely infers a rename ---");
{
	const plan = planNoEventStructuralRenames({
		missingCrdtPaths: [{ path: "Old/a.md", contentHash: "h1" }],
		extraDiskPaths: [{ path: "New/a.md", contentHash: "h1" }],
		mode: "authoritative",
		previousDiskIndex: {
			"Old/a.md": { mtime: 1, size: 10, contentHash: "h1" },
		},
	});
	assertEq(plan.renames.length, 1, "one baseline-backed rename planned");
	assertEq(plan.renames[0]?.reason, "baseline-hash-continuity", "reason records baseline continuity");
	assertEq(plan.unresolved.length, 0, "baseline-backed rename does not need attention");
}

console.log("\n--- No-event structural planner: baseline inference requires every safety condition ---");
{
	const cases = [
		{
			name: "authoritative mode",
			mode: "conservative" as const,
			previousDiskIndex: { "Old/a.md": { mtime: 1, size: 10, contentHash: "h1" } },
		},
		{
			name: "old path in prior index",
			mode: "authoritative" as const,
			previousDiskIndex: {},
		},
		{
			name: "matching prior baseline",
			mode: "authoritative" as const,
			previousDiskIndex: { "Old/a.md": { mtime: 1, size: 10, contentHash: "other" } },
		},
		{
			name: "new path absent from prior index",
			mode: "authoritative" as const,
			previousDiskIndex: {
				"Old/a.md": { mtime: 1, size: 10, contentHash: "h1" },
				"New/a.md": { mtime: 2, size: 10, contentHash: "h1" },
			},
		},
	];
	for (const testCase of cases) {
		const plan = planNoEventStructuralRenames({
			...
			extraDiskPaths: [{ path: "New/a.md", contentHash: "h1" }],
			mode: testCase.mode,
			previousDiskIndex: testCase.previousDiskIndex,
		});
		assertEq(plan.renames.length, 0, `${testCase.name} is required`);
		assertEq(plan.unresolved.length, 1, `${testCase.name} failure remains attention-worthy`);
	}
}
...
console.log("\n--- No-event structural planner: same content with different basename is not inferred as rename ---");
{
	const plan = planNoEventStructuralRenames({
		...
		mode: "authoritative",
		previousDiskIndex: {
			"Journal/2026-06-25.md": { mtime: 1, size: 10, contentHash: "same-template" },
		},
	});
...
console.log("\n--- No-event structural planner: duplicate content with unique basename is unresolved ---");
{
	const plan = planNoEventStructuralRenames({
		...
		mode: "authoritative",
		previousDiskIndex: {
			"Old/a.md": { mtime: 1, size: 10, contentHash: "same" },
			"Old/b.md": { mtime: 1, size: 10, contentHash: "same" },
		},
	});
...
console.log("\n--- No-event structural planner: explicit pairing does not relax original hash cardinality ---");
{
	const plan = planNoEventStructuralRenames({
		missingCrdtPaths: [
			{ path: "Old/a.md", contentHash: "same" },
	...
	assertEq(plan.renames.length, 1, "only the explicitly evidenced duplicate is renamed");
	assertEq(plan.renames[0]?.reason, "explicit-rename", "remaining 1:1 candidates are not baseline-inferred");
	assertEq(plan.unresolved.length, 1, "remaining duplicate candidates still need attention");
}
