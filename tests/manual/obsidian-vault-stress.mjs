...
const vaultRoot = process.argv[2] ?? "/home/user/vault";
...
const qaDir = path.join(vaultRoot, "kaos", "qa-stress");
const reportPath = path.join(
	"/tmp",
	`kaos-stress-report-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
);
...
async function readQuarantineState() {
	const pluginDataPath = path.join(vaultRoot, ".obsidian", "plugins", "kaos", "data.json");
	try {
