 ...
 *   KAOS_TEST_HOST   - Deployed server URL
 *   KAOS_TEST_TOKEN  - Bearer token for the server
 *   KAOS_TEST_VAULT_ID - Must be the same for both phases
 *
 * Optional env vars:
 *   KAOS_TEST_FILE_COUNT - Number of files (default: 700)
 *
 * Usage:
 *   # Phase 1: seed
 *   PHASE=seed KAOS_TEST_HOST=https://staging.workers.dev \
 *     KAOS_TEST_TOKEN=token KAOS_TEST_VAULT_ID=cold-test \
 ...
 *
 *   # Phase 2: validate
 *   PHASE=validate KAOS_TEST_HOST=https://staging.workers.dev \
 *     KAOS_TEST_TOKEN=token KAOS_TEST_VAULT_ID=cold-test \
 *     node --import jiti/register tests/manual/issue24-cold-load-validation.ts
 *
...
const VAULT_ID = process.env.KAOS_TEST_VAULT_ID;
if (!HOST || !TOKEN || !VAULT_ID) {
	console.error("Required: KAOS_TEST_HOST, KAOS_TEST_TOKEN, and KAOS_TEST_VAULT_ID env vars.");
	process.exit(1);
}
const FILE_COUNT = parseInt(process.env.KAOS_TEST_FILE_COUNT || "700", 10);
