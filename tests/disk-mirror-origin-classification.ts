...
assert(ORIGIN_EDITOR_AUTHORITY_SHIELD === "editor-authority-shield", "ORIGIN_EDITOR_AUTHORITY_SHIELD");
assert(
...
assert(
	ORIGIN_SAME_PATH_ADOPTION === "same-path-adoption",
	"ORIGIN_SAME_PATH_ADOPTION",
);
...
// Constants are identifiers, not string literals — this regex only fires on raw strings.
{
	...
	// enforcement (KaosLocalOrigin union type on the origin parameter) would be
