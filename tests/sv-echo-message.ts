...
console.log("\n--- Test 7: wrong schema version ---");
{
	...
	const wrongSchema = JSON.stringify({ type: "kaos/sv-echo", schema: 2, sv: encoded });
	assert(parseSvEchoMessage(wrongSchema) === null, "schema 2 => null");
	const zeroSchema = JSON.stringify({ type: "kaos/sv-echo", schema: 0, sv: encoded });
	assert(parseSvEchoMessage(zeroSchema) === null, "schema 0 => null");
	const strSchema = JSON.stringify({ type: "kaos/sv-echo", schema: "1", sv: encoded });
	assert(parseSvEchoMessage(strSchema) === null, "schema as string => null");
}

console.log("\n--- Test 8: missing or non-string sv field ---");
{
	const base = { type: "kaos/sv-echo", schema: 1 };
...
console.log("\n--- Test 9: oversized payload rejected ---");
{
	...
	const oversized = JSON.stringify({ type: "kaos/sv-echo", schema: 1, sv: oversizedB64 });
	...
	const atLimitMsg = JSON.stringify({ type: "kaos/sv-echo", schema: 1, sv: atLimit });
...
{
	const badB64 = JSON.stringify({ type: "kaos/sv-echo", schema: 1, sv: "not-valid-base64!!!" });
	assert(parseSvEchoMessage(badB64) === null, "invalid base64 in sv => null");
}

console.log("\n--- Test 11: decoded-but-invalid state vectors are rejected ---");
{
	const zeroBytes = JSON.stringify({ type: "kaos/sv-echo", schema: 1, sv: encodeBytesBase64(new Uint8Array(0)) });
...
console.log("\n--- Test 13: detailed parser reports reasons and counters ---");
{
	...
	const wrongSchema = parseSvEchoMessageDetailed(JSON.stringify({ type: "kaos/sv-echo", schema: 2, sv: encoded }));
